...
| 스프린트 | 기간 | 범위 |
|----------|------|------|
| Sprint 1 | 2주 | 프로젝트 기반 설정 + 인증 + DB 설계 ✅ 완료 |
| Sprint 2 | 2주 | OCR 인식 + 약품 정보 파싱 ✅ 완료 |
| Sprint 3 | 2주 | 부작용 보고 + 내 복용약 관리 + AI 분석 (MVP 완성) |
| Sprint 4~ | — | 보고 이력 조회 / PDF 공유 / 알림 / 예측 모델 / 성능 최적화 (추후 진행) |

- **현재 MVP 범위**: Sprint 1 ~ 3
- **추후 진행**: Sprint 4~ (보고 이력, PDF, 알림, 예측 모델, 통계, 운영 안정화)
...
## Sprint 4~ — 추후 진행 예정 (Post-MVP)

> 아래 항목은 MVP 완성 후 별도 계획으로 진행합니다.

- **보고 이력 조회** — 날짜/약품/증상 필터, 페이지네이션
- **의료진 공유** — PDF 보고서 생성, 공유 링크 (토큰 기반)
- **알림** — 웹 푸시 알림, 복용 시간 알림 스케줄러
- **부작용 예측 모델** — KAERS 데이터 기반 이상 탐지, 통계 시각화
- **성능/보안/운영** — Rate Limiting, OWASP 점검, 모니터링, DB 백업

---

<!-- ── 원래 Sprint 4 상세 내용 (추후 참고용) ─────────────────────────────
